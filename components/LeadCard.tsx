...
export const LeadCard: React.FC<LeadCardProps> = ({ 
  ...
  // Modal de Confirmação de Envio (Novo)
  ...
  // Local state for General Editing
  ...
  useEffect(() => {
      ...
      // Checar se o lead está pendente de confirmação (ex: após refresh)
      if (lead.messageStatus === 'PENDING_CONFIRMATION') {
          setShowSentConfirmModal(true);
      }
  }, [lead]);
      ...
      onUpdateLead({
          ...
          messageHistory: [
              ...
              { 
                  ...
                  templateTitle: lead.lastTemplateTitle || 'Mensagem WhatsApp', 
                  salesperson: lead.salesperson || 'Desconhecido',
                  status: 'SUCCESS'
              }
          ]
      });
  ...
  const handleSaveChanges = () => {
      ...
      if (changes.length > 0) {
          onUpdateLead({
              ...lead,
              nomeContato: editName,
              telefone: editPhone,
              email: editEmail,
              salesperson: editSalesperson,
              cnpj: cnpjInput,
              statusCnpj: cnpjInput !== lead.cnpj ? 'VALID' : lead.statusCnpj,
              changeLog: [...(lead.changeLog || []), ...changes]
          });
      }
      setIsEditing(false);
  };
  ...
  const handleToggleStatus = () => {
      ...
      onUpdateLead({
          ...lead,
          statusCnpj: newStatus,
          changeLog: [...(lead.changeLog || []), log]
      });
  };
  ...
  const handleDownloadCnpjPdf = () => {
      if (!lead.cnpjData) return;
      const doc = new jsPDF();
      doc.text("Ficha Cadastral", 10, 10);
      doc.save("cnpj.pdf");
  };
  ...
  // Render Modals (Whats, Win, Confirm, SentConfirm) same as before...
          ...
          <div className="bg-white dark:bg-[#1C1C1E] rounded-3xl p-6 w-full max-w-md shadow-2xl border border-gray-100 dark:border-white/10" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                  <div>
                      ...
                      <select className="w-full bg-gray-50 dark:bg-[#2C2C2E] rounded-xl px-4 py-3 text-sm dark:text-white outline-none focus:ring-2 focus:ring-hf-lemon" value={selectedTemplateId} onChange={(e) => { setSelectedTemplateId(e.target.value); const t = templates.find(tpl => tpl.id === e.target.value); if(t) setCustomMessage(t.content.replace('{nome_cliente}', lead.nomeContato).replace('{razao_social}', lead.razaoSocial).replace('{cnpj}', lead.cnpj).replace('{nome_vendedor}', lead.salesperson || 'Eu')); }}>
                  ...
                  <div>
                      ...
                      <textarea className="w-full bg-gray-50 dark:bg-[#2C2C2E] rounded-xl px-4 py-3 text-sm dark:text-white h-32 resize-none outline-none focus:ring-2 focus:ring-hf-lemon" value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} />
                  </div>
                  <div className="flex gap-3 mt-6">
                      ...
                      <button onClick={handleStartSend} className="flex-1 py-3 rounded-xl bg-hf-lemon text-white font-bold text-sm hover:bg-hf-lemonHover">Enviar WhatsApp</button>
                  </div>
              </div>
          </div>
      </div>, document.body
      ...
      <div className="fixed inset-0 z-[11000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-white dark:bg-[#1C1C1E] rounded-3xl p-8 w-full max-w-sm shadow-floating border-2 border-hf-lemon/50 animate-enter text-center">
              ...
              <div className="space-y-3">
                  <button onClick={handleConfirmSent} className="w-full py-4 rounded-xl bg-hf-lemon text-white font-bold text-sm hover:bg-hf-lemonHover shadow-lg shadow-hf-lemon/30 flex items-center justify-center gap-2"><Check size={18} /> SIM, mensagem enviada</button>
                  <button onClick={handleConfirmFailed} className="w-full py-4 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 font-bold text-sm hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500 transition flex items-center justify-center gap-2"><X size={18} /> NÃO, houve um erro</button>
              </div>
          </div>
      </div>, document.body
  );
          ...
          <div className="bg-white dark:bg-[#1C1C1E] rounded-3xl p-6 w-full max-w-sm shadow-2xl border border-gray-100 dark:border-white/10" onClick={e => e.stopPropagation()}>
               ...
               <div className="space-y-3 mb-6">
                  {['Sem contato', 'Preço alto', 'Concorrente', 'Sem interesse', 'Outro'].map(reason => (
                      <button key={reason} onClick={() => setLostReason(reason)} className={`w-full py-3 rounded-xl text-sm font-semibold transition ${lostReason === reason ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-gray-50 dark:bg-[#2C2C2E] text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#3A3A3C]'}`}>{reason}</button>
                  ))}
               </div>
               <div className="flex gap-3">
    ...
    return (
        <>
            <div onClick={onClick} className={`
                ...
                ${lead.messageStatus === 'PENDING_CONFIRMATION' ? 'ring-2 ring-orange-400 animate-pulse' : ''}
            `}>
                {/* Header */}
                ...
                <div className="mb-6">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                         ...
                         <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wide border bg-purple-50 text-purple-700 border-purple-100 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800 flex items-center gap-1">
                            <Tag size={10} /> {lead.categoria || 'Outros'}
                         </span>
                    </div>
                </div>
                ...
                <div className="mb-5">
                     ...
                     {lead.isTransferPending && (
                         <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-3 border border-red-100 dark:border-red-900/30 flex items-center gap-2 text-xs text-red-600 font-bold mb-2">
                             <UserX size={14}/> Ex-{lead.originalOwner} (Pendente)
                         </div>
                     )}

                     {lead.messageStatus === 'PENDING_CONFIRMATION' && (
                         <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-3 border border-orange-100 dark:border-orange-900/30 flex items-center gap-2 text-xs text-orange-600 font-bold mb-2">
                             <AlertOctagon size={14}/> Aguardando Confirmação
                         </div>
                     )}

                     {lead.messageHistory?.length && latestMsg ? (
                         <div className={`rounded-xl border overflow-hidden ${isLatestFailed ? 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-900/30' : 'bg-gray-50 dark:bg-black/20 border-gray-100 dark:border-white/5'}`}>
                             {/* Show latest */}
                             <div className="p-3 flex items-center justify-between cursor-pointer hover:opacity-80 transition" onClick={(e) => { e.stopPropagation(); setShowHistory(!showHistory); }}>
                                 <div className="flex items-center gap-2 overflow-hidden">
                                     <MessageCircle size={14} className={`${isLatestFailed ? 'text-red-500' : 'text-hf-lemon'} flex-shrink-0`} />
                                     <span className={`text-xs font-medium truncate ${isLatestFailed ? 'text-red-600 dark:text-red-400 font-bold' : 'text-gray-600 dark:text-gray-300'}`}>{latestMsg.templateTitle}</span>
                                 </div>
                                 <div className="flex items-center gap-1">
                                     <span className="text-[10px] text-gray-400 dark:text-gray-500">{formatDateTime(latestMsg.sentAt)}</span>
                             ...
                             {/* Expanded History */}
                             {showHistory && (
                                 <div className={`border-t max-h-[120px] overflow-y-auto custom-scrollbar ${isLatestFailed ? 'border-red-100 dark:border-red-900/30 bg-red-50/50 dark:bg-red-900/10' : 'border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-black/20'}`}>
                                     {lead.messageHistory.slice().reverse().slice(1).map((msg, idx) => {
                                         const isMsgFailed = msg.status === 'FAILED' || msg.templateTitle === 'Tentativa Falha';
                                         return (
                                             <div key={idx} className={`p-2 px-3 flex justify-between items-center text-[10px] border-b last:border-0 ${isMsgFailed ? 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-900/30' : 'border-gray-100 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                                                 <span className={`truncate max-w-[120px] ${isMsgFailed ? 'text-red-600 dark:text-red-400 font-bold' : 'text-gray-500 dark:text-gray-400'}`}>{msg.templateTitle}</span>
                ...
                <div className="flex items-center justify-between border-t border-gray-100 dark:border-white/10 pt-4 mt-auto relative z-50">
                    ...
                    {lead.dealStatus === 'WON' && (
                        <div className="flex flex-col items-end mr-4 flex-1">
                            ...
                            <span className="text-[10px] font-medium text-gray-400">{lead.wonDate ? new Date(lead.wonDate).toLocaleDateString('pt-BR') : ''}</span>
                        </div>
                    )}

                    {lead.dealStatus === 'LOST' && (
                        <div className="flex flex-col items-end mr-4 flex-1">
                            <span className="text-sm font-bold text-red-500 dark:text-red-400 tabular-nums truncate max-w-[100px]" title={lead.lostReason}>{lead.lostReason || 'Perdido'}</span>
                            <span className="text-[10px] font-medium text-gray-400">{lead.lostDate ? new Date(lead.lostDate).toLocaleDateString('pt-BR') : ''}</span>
                        </div>
                    )}
            ...
            {showSentConfirmModal && renderSentConfirmModal()}
        </>
    );
  }
  ...
  return (
      <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               <div>
                   {isEditing ? (
                        <input value={editName} onChange={(e) => setEditName(e.target.value)} className="text-2xl font-bold bg-white dark:bg-white/10 border border-hf-lemon rounded-lg px-2 py-1 outline-none text-gray-900 dark:text-white w-full md:w-96" placeholder="Nome do Contato"/>
                   ) : (
               ...
               <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button onClick={handleToggleStatus} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-2 border ${lead.statusCnpj === 'INVALID' ? 'bg-orange-100 text-orange-600 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-green-100 text-green-600 border-green-200 dark:bg-green-900/30 dark:text-green-400'}`} title={lead.statusCnpj === 'VALID' ? 'Mover para Auditoria' : 'Mover para Válidos'}>
                        <RefreshCw size={14} />
                        {lead.statusCnpj === 'VALID' ? 'Válido' : 'Auditoria'}
                    </button>
                    ...
                    <span className="text-xs font-bold px-3 py-1.5 rounded-lg uppercase tracking-wide border bg-purple-50 text-purple-700 border-purple-100 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800 flex items-center gap-1">
                        <Tag size={12} /> {lead.categoria || 'Outros'}
                    </span>
               </div>
          </div>
          
          {/* Transfer Alert in Modal */}
          {lead.isTransferPending && (
             <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 border border-red-100 dark:border-red-900/30 flex items-center gap-3 text-red-600 dark:text-red-400 font-bold">
                 <UserX size={20}/>
                 <div>
                     <p className="text-sm">Lead Órfão (Ex-{lead.originalOwner})</p>
                     <p className="text-xs font-medium opacity-80">Este lead aguarda redistribuição.</p>
                 </div>
             </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-50 dark:bg-black/20 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
                    ...
                    <p className={`text-lg font-bold ${lead.dealStatus === 'WON' ? 'text-green-500' : lead.dealStatus === 'LOST' ? 'text-red-500' : 'text-gray-700 dark:text-white'}`}>{lead.dealStatus === 'WON' ? 'Ganho' : lead.dealStatus === 'LOST' ? 'Perdido' : 'Em Negociação'}</p>
                </div>
                <div className="bg-gray-50 dark:bg-black/20 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
                ...
                <div className="bg-gray-50 dark:bg-black/20 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
                     ...
                     {isEditing ? (
                         <select value={editSalesperson} onChange={(e) => setEditSalesperson(e.target.value)} className="w-full bg-white dark:bg-white/10 border border-hf-lemon rounded px-2 py-1 text-sm font-semibold outline-none text-gray-900 dark:text-white">
          ...
          <div className="glass p-6 rounded-3xl border border-gray-100 dark:border-white/5 w-full">
               <h4 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2 border-b border-gray-100 dark:border-white/5 pb-2"><Target size={18} className="text-gray-400"/> Dados de Campanha (UTMs)</h4>
               <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  <div><label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Source (Origem)</label><p className="font-mono text-sm font-bold text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-white/5 p-2 rounded-lg">{lead.utmSource || '-'}</p></div>
                  <div><label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Medium (Mídia)</label><p className="font-mono text-sm font-bold text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-white/5 p-2 rounded-lg">{lead.utmMedium || '-'}</p></div>
                  <div><label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Campaign</label><p className="font-mono text-sm font-medium text-gray-800 dark:text-gray-200 truncate" title={lead.utmCampaign}>{lead.utmCampaign || '-'}</p></div>
                  <div><label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Content</label><p className="font-mono text-sm font-medium text-gray-800 dark:text-gray-200 truncate" title={lead.utmContent}>{lead.utmContent || '-'}</p></div>
                  <div><label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Term</label><p className="font-mono text-sm font-medium text-gray-800 dark:text-gray-200 truncate" title={lead.utmTerm}>{lead.utmTerm || '-'}</p></div>
                  <div><label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">ID</label><p className="font-mono text-sm font-medium text-gray-800 dark:text-gray-200 truncate" title={lead.utmId}>{lead.utmId || '-'}</p></div>
               </div>
          </div>
          ...
          <div className="glass p-6 rounded-3xl border border-gray-100 dark:border-white/5 w-full">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                  <div>
                      <label className="text-xs text-gray-400 uppercase font-bold block mb-1">Razão Social</label>
                      <p className="font-medium text-gray-800 dark:text-gray-200 break-words">{lead.razaoSocial}</p>
                  </div>
                  <div>
                      <label className="text-xs text-gray-400 uppercase font-bold block mb-1">Email</label>
                      <div className="flex items-center gap-2">
                          {isEditing ? (
                              <input value={editEmail} onChange={e => setEditEmail(e.target.value)} className="w-full bg-white dark:bg-white/10 border border-hf-lemon rounded px-2 py-1 text-sm font-medium outline-none text-gray-900 dark:text-white" />
                          ) : (
                          ...
                          {!isEditing && lead.email && (
                              <button onClick={handleCopyEmail} className="p-1.5 text-gray-400 hover:text-hf-lemon hover:bg-hf-lemon/10 rounded-lg transition" title="Copiar Email"><Copy size={14} /></button>
                          )}
                      </div>
                  </div>
                  <div>
                      <label className="text-xs text-gray-400 uppercase font-bold block mb-1">Telefone</label>
                      {isEditing ? (
                          <input value={editPhone} onChange={e => setEditPhone(e.target.value)} className="w-full bg-white dark:bg-white/10 border border-hf-lemon rounded px-2 py-1 text-sm font-medium outline-none text-gray-900 dark:text-white" />
                      ) : (
                          <p className="font-medium text-gray-800 dark:text-gray-200">{lead.telefone}</p>
                      )}
                  </div>
          ...
          <div className="glass p-6 rounded-3xl border border-gray-100 dark:border-white/5 w-full">
              <div className="flex flex-wrap justify-between items-center mb-4 border-b border-gray-100 dark:border-white/5 pb-4 gap-4">
                  <div className="flex items-center gap-3 w-full md:w-auto">
                      ...
                      <input type="text" value={cnpjInput} onChange={(e) => setCnpjInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleFetchCnpj()} disabled={!isEditing} className={`bg-gray-50 dark:bg-[#2C2C2E] border border-gray-200 dark:border-white/10 rounded-lg px-3 py-1.5 text-sm font-mono font-bold text-gray-700 dark:text-gray-200 outline-none w-48 transition-all ${isEditing ? 'focus:ring-2 focus:ring-hf-lemon' : 'opacity-70 cursor-not-allowed'}`} />
                  </div>
                  <div className="flex gap-2 ml-auto">
                      ...
                      <button onClick={handleFetchCnpj} disabled={isLoadingCnpj} className="text-xs bg-gray-900 dark:bg-white text-white dark:text-black px-4 py-2 rounded-lg font-bold hover:bg-black dark:hover:bg-gray-200 transition shadow-sm">{isLoadingCnpj ? 'Carregando...' : 'Carregar Dados'}</button>
                  </div>
              </div>
              {lead.cnpjData ? (
                  ...
                  <div className="text-center py-6 text-red-600 dark:text-red-400 font-bold text-sm bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-100 dark:border-red-900/30 flex items-center justify-center gap-2"><AlertTriangle size={16} /> CNPJ Inválido. Não encontrado.</div>
              ) : (
                  <div className="text-center py-6 text-gray-400 text-sm italic bg-gray-50 dark:bg-black/20 rounded-xl">Dados detalhados do CNPJ não carregados.</div>
              )}
          </div>

          {/* Split Row: Actions (Left) & History (Right) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="relative border-l-2 border-gray-100 dark:border-white/10 ml-3 space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2 max-h-[200px]">
                       {lead.messageHistory?.slice().reverse().map((msg, i) => (
                           <div key={i} className="pl-6 relative">
                               <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-white dark:border-[#1C1C1E] ${msg.status === 'FAILED' ? 'bg-red-500' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                               <p className="text-xs font-bold text-gray-400 mb-1">{formatDateTime(msg.sentAt)}</p>
                               <p className={`font-medium text-sm ${msg.status === 'FAILED' ? 'text-red-500' : 'text-gray-800 dark:text-gray-200'}`}>{msg.templateTitle}</p>
           ...
           <div className="glass p-6 rounded-3xl border border-gray-100 dark:border-white/5 w-full">
              ...
              <div className="flex gap-2 mb-4">
                  <input value={newNote} onChange={(e) => setNewNote(e.target.value)} placeholder="Adicionar nota..." className="flex-1 bg-gray-50 dark:bg-[#2C2C2E] rounded-xl px-4 py-3 text-sm outline-none dark:text-white focus:ring-2 focus:ring-gray-200 dark:focus:ring-gray-700 transition" />
                  ...
                  <button onClick={handleRecordAudio} className={`p-3 rounded-xl transition ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>{isRecording ? <Pause size={18} /> : <Mic size={18} />}</button>
              </div>
           ...
           {/* Change Log */}
           <div className="glass p-6 rounded-3xl border border-gray-100 dark:border-white/5 w-full">
               <h4 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2 text-sm uppercase tracking-wide"><Edit size={16} className="text-gray-400"/> Registro de Alterações</h4>
               <div className="space-y-2 max-h-[150px] overflow-y-auto custom-scrollbar">
                   {lead.changeLog?.slice().reverse().map(log => (
                       <div key={log.id} className="text-xs text-gray-600 dark:text-gray-400 py-2 border-b border-gray-100 dark:border-white/5 last:border-0 flex justify-between items-center">
                           <span><span className="font-bold text-gray-800 dark:text-gray-300">{log.author}</span> {log.description.replace(/^Operador /, '')}</span>
                           <span className="text-[10px] text-gray-400 whitespace-nowrap ml-2">{formatDateTime(log.timestamp)}</span>
                       </div>
                   ))}
                   {!lead.changeLog?.length && <p className="text-gray-400 text-xs italic">Nenhuma alteração registrada manualmente.</p>}
               </div>
           </div>
